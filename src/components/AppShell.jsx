                  ...
                  {checklistRows.map((row) => (
                    <div key={row.id} className="row-card">
                      ...
                      <div className="actions-end">
                        ...
                        <span style={{ textAlign: 'center', justifyContent: 'center' }} className={`badge ${row.submission?.approvedByClient ? 'success' : 'outline'}`}>{row.submission?.approvedByClient ? 'Aprobado' : 'Sin aprobar'}</span>
                        <button className="secondary" style={{ fontSize: '12px', padding: '6px 12px' }} onClick={() => { setSelectedDocType(row); setSubmissionOpen(true); }}>Registrar</button>
