                  ...
                  {checklistRows.map((row) => (
                    <div key={row.id} className="row-card">
                      ...
                      <div className="actions-end">
                        <span style={{ width: '105px', justifyContent: 'flex-start' }} className={`badge ${row.submission?.delivered ? '' : 'outline'}`}>{row.submission?.delivered ? 'Entregado' : 'No entregado'}</span>
                        <span style={{ width: '105px', justifyContent: 'flex-start' }} className={`badge ${row.submission?.approvedByClient ? 'success' : 'danger'}`}>{row.submission?.approvedByClient ? 'Aprobado' : 'Sin aprobar'}</span>
                        <button className="secondary" style={{ width: '105px', textAlign: 'left', fontSize: '12px', padding: '6px 12px' }} onClick={() => { setSelectedDocType(row); setSubmissionOpen(true); }}>Registrar</button>
                        <button title="Eliminar" style={{ background: 'transparent', padding: '8px', display: 'grid', placeItems: 'center', color: '#64748b' }} onClick={() => deleteDocumentType(row.id)}>
